...
public class QLearning {
    ...
    static void playGame(GameBoard gameBoard) {
        ...
        while (!gameBoard.isGameFinished()) {
            // Whites' turn
            int[] currentState = convertGameBoardToState(gameBoard);
            ...
            updateWeights(currentState, whiteNextState, whiteReward);
            ...
            int[] nextState = convertGameBoardToState(gameBoard);
            ...
            updateWeights(nextState, blackNextState, blackReward);
        }
    }
    ...
    static Move getRandomAction(GameBoard gameBoard, boolean isWhitesTurn) {
        ...
        Piece piece;
        if (isWhitesTurn) {
            piece = gameBoard.getWhitesList().get(rand.nextInt(gameBoard.getWhitesList().size()));
        } else {
    ...
    static void updateWeights(int[] currentState, int[] nextState, double reward) {
        double currentQValue = calculateQValue(currentState);
        double maxNextQValue = getMaxQValue(nextState);

        for (int i = 0; i < weights.length; i++) {
            weights[i] = weights[i] + alpha * (reward + gamma * maxNextQValue - currentQValue) * currentState[i];
        }
    }
